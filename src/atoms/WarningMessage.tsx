...
export default function WarningMessage({ gameMechanics }: WarningMessageProps) {
  ...
  const getLetterConditions = () => {
    ...
    if (letters.length === 1)
      return t("warningMessage.thereIsOne", { condition: letters[0] });
    if (letters.length === 2)
      return t("warningMessage.thereAreTwoAnd", {
        first: letters[0],
        second: letters[1],
      });
  ...
  if (gameMechanics.first && gameMechanics.mirrored) {
    conditions.push(t("warningMessage.lettersHiddenAndMirrored"));
  } else {
    if (gameMechanics.first) conditions.push(t("warningMessage.wordsHidden"));
    if (gameMechanics.mirrored)
      conditions.push(t("warningMessage.lettersMirrored"));
  }
  ...
  const formatWithStyles = (text: string) => {
    return text
      ...
      .map((word, index) => {
        ...
        let styledWord;
        if (cleanWord === t("warningMessage.fake"))
          styledWord = (
            <span key={index} className="lost">
              {cleanWord}
            </span>
          );
        else if (cleanWord === t("warningMessage.hidden"))
          styledWord = (
            <span key={index} className="highlight">
              {cleanWord}
            </span>
          );
        else if (cleanWord === t("warningMessage.dark"))
          styledWord = (
            <span key={index} className="dark">
              {cleanWord}
            </span>
          );
        else if (cleanWord === t("warningMessage.hide"))
          styledWord = (
            <span key={index} className="won">
              {cleanWord}
            </span>
          );
        else if (cleanWord === t("warningMessage.still"))
          styledWord = (
            <span key={index} className="still">
              {cleanWord}
            </span>
          );
        else if (
          ...
          styledWord = (
            <span key={index} className="mirrored">
              {cleanWord}
            </span>
          );
        else if (cleanWord === "OCULTAN")
          styledWord = (
            <span key={index} className="won">
              {cleanWord}
            </span>
  ...
  return (
    <>
      <h4>
        {conditions.length > 0 && t("warningMessage.warning")}
        {formatWithStyles(getFormattedText())}
      </h4>
    </>
  );
}
