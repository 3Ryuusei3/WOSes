...
export default function useRealtimeConnection(
  ...
  const updateStatus = useCallback((newStatus: ConnectionStatus) => {
    console.log('[RealtimeConnection] Status changed:', globalConnectionStatus, '->', newStatus);
  ...
  useEffect(() => {
    ...
    if (!globalChannel) {
      console.log('[RealtimeConnection] Creating global connection monitor');
      globalChannel = supabase
        .channel('connection-monitor-global')
        .on('system', {}, (payload: any) => {
          console.log('[RealtimeConnection] System event:', payload);
          if (payload.status === 'CHANNEL_ERROR' || payload.status === 'TIMED_OUT') {
            if (globalConnectionStatus !== 'reconnecting' && globalConnectionStatus !== 'disconnected') {
              updateStatus('disconnected');
              attemptReconnect();
            }
          } else if (payload.status === 'SUBSCRIBED' && globalConnectionStatus !== 'connected') {
            updateStatus('connected');
            setRetryCount(0);
          }
        })
    ...
    return () => {
      ...
      if (listeners.size === 0 && globalChannel) {
        console.log('[RealtimeConnection] Cleaning up global monitor');
