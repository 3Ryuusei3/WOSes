...
export async function createRoomWithHost(
  ...
  const payload = data as { room_id: number; code: string; state: string; players: string; host_player_id: number };

  return {
    data: {
      room: { id: payload.room_id, code: payload.code, state: payload.state, players: payload.players },
      host: { id: payload.host_player_id, room_id: payload.room_id, name: hostName, score: 0, role: 'host' }
    },
    error: null
  };
}
...
export async function joinRoomAsPlayer(
  ...
  const payload = data as { room_id: number; code: string; state: string; players: string; player_id: number; player_name: string };

  return {
    data: {
      room: { id: payload.room_id, code: payload.code, state: payload.state, players: payload.players },
      player: { id: payload.player_id, room_id: payload.room_id, name: payload.player_name, score: 0, role: 'player' }
    },
    error: null
  };
}

export async function getRoomPlayers(roomId: number): Promise<DbResponse<RoomPlayerRow[]>> {
...
export async function startRoom(code: string): Promise<DbResponse<RoomRow>> {
  ...
  if (error) return { data: null, error };
  const payload = data as { room_id: number; code: string; state: string; players: string; current_word?: string | null };
...
export async function advanceRoomToGame(code: string): Promise<DbResponse<RoomRow>> {
  const { data, error } = await supabase.rpc('advance_room_to_game', { p_code: code });
  if (error) return { data: null, error };
  const payload = data as { room_id: number; code: string; state: string; players: string; current_word?: string | null };
  return { data: { id: payload.room_id, code: payload.code, state: payload.state, players: payload.players, current_word: payload.current_word }, error: null };
}

export async function startRoomWithWord(code: string, currentWord: string): Promise<DbResponse<RoomRow>> {
  const { data, error } = await supabase.rpc('start_room_with_word', { p_code: code, p_current_word: currentWord });
  if (error) return { data: null, error };
  const payload = data as { room_id: number; code: string; state: string; players: string; current_word?: string | null; round_id?: number };
  return { data: { id: payload.room_id, code: payload.code, state: payload.state, players: payload.players, current_word: payload.current_word }, error: null };
}
...
export async function getCorrectWords(roomId: number, roundId?: number): Promise<DbResponse<RoomWordRow[]>> {
...
export async function getPlayerNameById(playerId: number): Promise<DbResponse<{ name: string }>> {
  const { data, error } = await supabase
    .from('room_players')
    .select('name')
    .eq('id', playerId)
...
export async function finishRoundToLobby(code: string): Promise<DbResponse<RoomRow>> {
  const { data, error } = await supabase.rpc('finish_round_to_lobby', { p_code: code });
  if (error) return { data: null, error };
  const payload = data as { room_id: number; code: string; state: string; players: string };
  return { data: { id: payload.room_id, code: payload.code, state: payload.state, players: payload.players }, error: null };
}

export async function finishRoundToLost(code: string): Promise<DbResponse<RoomRow>> {
  const { data, error } = await supabase.rpc('finish_round_to_lost', { p_code: code });
  if (error) return { data: null, error };
  const payload = data as { room_id: number; code: string; state: string; players: string };
  return { data: { id: payload.room_id, code: payload.code, state: payload.state, players: payload.players }, error: null };
}
...
export async function getLatestRoundId(roomId: number): Promise<DbResponse<number>> {
  const { data, error } = await supabase
    .from('rounds')
    .select('id')
    .eq('room_id', roomId)
...
export async function getCorrectCountForRound(roomId: number, roundId: number): Promise<DbResponse<number>> {
  const { data, error } = await supabase
    .from('room_words')
  ...
  return { data: (data as any)?.length ?? (data as any)?.count ?? 0, error: null };
}
